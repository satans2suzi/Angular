...
let generateAuthToken = async payload => {
    const {id, username, role, fullname} = payload;
    // Create JWT
    ...
    const [encode_accToken, encode_rfToken] = await Promise.all([
        jwt.sign({
            ...
            username: username,
            fullname: fullname
        }, process.env.JWT_KEY, {expiresIn: process.env.ACC_TOKEN_TIME}),
        jwt.sign({
            id: id,
            role: role,
            username: username,
            fullname: fullname
        }, process.env.JWT_KEY_REFRESH, {expiresIn: process.env.RF_TOKEN_TIME})
    ]);
...
class UsersController {
    ...
    async checkUser(req, res) {
        try {
            const awUsername = await userModel.findOne(req.body);
            if (awUsername) {
                return res.status(200).send({
                    message: apiStatusCode.status200('Tên tài khoản đã được sử dụng!.'),
                    data: false
                });
            }
            return res.status(200).send({
                message: apiStatusCode.status200('Tên tài khoản Chưa được sử dụng!.'),
                data: true
            });
        } catch (e) {
    ...
    async checkEmail(req, res) {
        try {
            const awEmail = await userModel.findOne(req.body);
            if (awEmail) {
                return res.status(200).send({
                    message: apiStatusCode.status200('Email đã được sử dụng!.'),
                    data: false
                });
            }
            return res.status(200).send({
                message: apiStatusCode.status200('Email chưa được sử dụng!.'),
                data: true
            });
        } catch (e) {
            return res.status(500).send({message: apiStatusCode.status500('Lỗi Server!.')});
